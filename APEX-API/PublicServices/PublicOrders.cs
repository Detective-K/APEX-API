        ...
        public void GBResult(dynamic OData, string type, string addbuy)
        {
            ...
            if (Convert.ToString(type) == "5")
            {
                ...
                if (Convert.ToString(MortorInfo.FirstOrDefault().TcOek27) == "Y")  //Y:反鎖  N:正鎖
                {
                    ...
                    ScrewDia = _publicFunction.Get_C12(LZ1);
                }
                else
                {
                    Motor_Screw_orientation = "N";
                    ScrewDia = _publicFunction.Get_M(Convert.ToDouble(MortorInfo.FirstOrDefault().TcOek15));   //20180130改用LZ去反查ScrewDia,不在用表格的ScrewDia
                }
                ...
                LZ = !String.IsNullOrEmpty(Convert.ToString(MortorInfo.FirstOrDefault().TcOek15).Trim()) ? Convert.ToString(MortorInfo.FirstOrDefault().TcOek15).Replace(".000", "") : _publicFunction.Get_C12_NEW(LZ1);
            ...
            TcShwInfo = _orderService.GetTcShwFileInfo(l_suitadapter);
            ...
            if (Convert.ToDouble(LR) < tc_shw10 + _publicFunction.Ceiling(Convert.ToDouble(LT), 0.5) + 0.5)
            {
            ...
            {
                if (Convert.ToDouble(LA) - Convert.ToDouble(_publicFunction.Get_M(Convert.ToDouble(LZ))) - tc_shw08 < 1 && _publicFunction.getScrewT(_publicFunction.Get_M(Convert.ToDouble(LZ))) > Convert.ToDouble(LT))
                {
                    //B1 +SCREW_DEP +0.5 > Motor.LR
                    if (tc_shw10 + _publicFunction.getScrewT(_publicFunction.Get_M(Convert.ToDouble(LZ))) + 0.5 > Convert.ToDouble(LR))
                    {
                        errMsg = "Error !";//Resources.Resource.msg020
                    }
                }
            ...
            {
                if (Convert.ToDouble(LA) - _publicFunction.Get_Screw_Max_Dia(Convert.ToDouble(_publicFunction.Get_C12_NEW(LZ1))) - 2 < Reducer_D9)
                {
            ...
            if (Motor_Screw_orientation == "N")
            {
                if (Convert.ToDouble(LA) - Convert.ToDouble(_publicFunction.Get_M(Convert.ToDouble(LZ))) - Convert.ToDouble(LB) < 2)
                {
            ...
            {
                if (Math.Abs(Convert.ToDouble(LA) - Convert.ToDouble(_publicFunction.Get_C12_NEW(LZ1)) - Convert.ToDouble(LB)) < 2)
                {
                    errMsg = "Error !";// Resources.Resource.msg022
                }
            }
